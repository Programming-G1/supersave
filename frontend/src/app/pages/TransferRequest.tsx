...
export default function TransferRequest() {
  ...
  const { hospitals, patientTemplate, loading, error } = useEmergencyAppData();
  ...
  useEffect(() => {
    if (!patientTemplate || patientData.name) {
      return;
    }
    ...
    setPatientData({
      ...
      gender: patientTemplate.gender,
      symptoms: patientTemplate.symptoms,
      severity: patientTemplate.severity,
      bloodPressure: patientTemplate.vitalSigns.bloodPressure,
      heartRate: patientTemplate.vitalSigns.heartRate,
      temperature: patientTemplate.vitalSigns.temperature,
      oxygenSaturation: patientTemplate.vitalSigns.oxygenSaturation,
    });
  }, [patientTemplate, patientData.name]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await emergencyApi.createTransfer(selectedHospitalId, {
        id: patientTemplate?.id ?? 'p1',
      ...
      setEta(response.estimatedTime);
      toast.success('이송이 시작되었습니다!', {
        description: response.message,
      });
    } catch (apiError) {
      setIsSubmitting(false);
      toast.error(apiError instanceof Error ? apiError.message : '이송 요청에 실패했습니다.');
    }
  };
  ...
  if (loading) {
    return (
      <Card className="p-8 text-center">
        <p className="text-gray-500">이송 요청 화면을 불러오는 중입니다...</p>
      </Card>
    );
  }
            ...
            <Card className="p-6">
              ...
              {!selectedHospitalId ? (
                ...
                <div className="space-y-4">
                  ...
                  {selectedHospital && <HospitalCard hospital={selectedHospital} showActions={false} />}
                  <Button
                    type="button"
                    variant="outline"
